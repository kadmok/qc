...
getextentsingle<- function(x1, data){
  #print(data %>% dplyr::filter(x==x1))
 # print(filter(data, x==x1))
  #data %>% dplyr::filter(x==x1) %>% dplyr::distinct(y) )
 # data<-data %>% dplyr::filter(x==x1)
  #data<-filter(data, x==x1)
  #data<-unique(data)
 # return(filter(data, x==x1)%>% dplyr::distinct(y))
  return(data %>% dplyr::filter(x==x1) %>% dplyr::distinct(y) )
}
#getextentsingle("e",datedges)
t1<-Sys.time()
getintent3("V1c", datedges)
print( Sys.time()- t1)
...
getintent3<- function(x, data){
  ...
  for (i in extent$y){
    ...
    for (n in intentsingle){
      intent<-lastloop_getintent(n,data,extent,extsize)
      returnlist<-c(returnlist, intent)
    }   
    time<-time+ Sys.time()-a
  }
 # print("llp")
 # print(time)
  return (unlist(unique(returnlist)))}
...
lastloop_getintent<- function(n, data, ext, extsize){
  ...
  #this one is fast
  trfalse<-length( ext$y[na.omit(match(ext$y, extentsingle$y))] )==extsize
  #print( Sys.time()-a)
  if (trfalse)  {  
    return(paste("",n,sep=""))
  }}
...
lenintent_equals_support<-function(lenintent, xn, extent){
  xn<-paste("",xn,sep="")
for (e in 1:length(extent[[1]] )){
  p<- paste("i",(extentht[[xn]])[e],sep="")
  if(!is.null( supporty[[p]])){
  if(supporty[[p]][1]==lenintent){
    return( TRUE)
  }}
}
  return(FALSE)
...
qcover3_noprint<- function(tabl, itemcount){
  ...
  for(i in 1:nrow(tabl)){
    ...
    #print(lenintent_equals_support(tabl$len_intent[i], tabl$item[i],tabl$extent[i]))
    if (tabl$len_extent[i]==1 | lenintent_equals_support(tabl$len_intent[i], tabl$item[i],tabl$extent[i])){
      num=num+1
